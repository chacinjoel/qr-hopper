import {AudioDecoder,synthesize} from './audio-codec.js?v=rxfix2';
 ...
 async enable({microphone=false}={}){
  ...
  if(microphone&&!this.stream){
   ...
   try{this.onState('Esperando permiso del micrófono…');let expired=false;
    ...
    this.onState('Cargando analizador de tonos…');await bounded(this.context.audioWorklet.addModule(new URL('./audio-worklet.js?v=rxfix2',import.meta.url)),10000,'No se pudo iniciar el analizador de audio. Usa QR.');
