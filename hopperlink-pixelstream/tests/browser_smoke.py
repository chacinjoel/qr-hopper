"""Legacy browser smoke + optical loopback. Synthetic camera, not physical phone test."""
...
url=f'http://127.0.0.1:{server.server_port}/hopperlink-pixelstream/legacy.html'
...
with sync_playwright() as pw:
    ...
    result['tests'].append('legacy HTML/modules boot without errors')
    ...
    result['tests'].append('legacy preparation, transmission and exit')
    ...
    page.evaluate('''async()=>{
      ...
      let i=0;window.testInterval=setInterval(()=>{const f=transport.frames[i];renderOpticalFrame(canvas,f,transport,p,i,transport.frames.length);i=(i+1)%transport.frames.length;},180);
    }''')
    ...
    assert not errors,errors
    result['tests'].append('legacy renderer -> tracker -> FEC -> SHA-256')
    page.locator('#cameraBtn').click()
