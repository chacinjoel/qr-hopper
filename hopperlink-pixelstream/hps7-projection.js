...
function render(){const panel=ensurePanel();if(!panel)return;if(hps8Mode()){panel.style.display='none';return;}const total=numTotal();if(!total){panel.style.display='none';return;}const grid=Number($('gridSize')?.value||56),dual=isDual(),sel=selectedBits(),rows=[2,3,4].map(bits=>projection(total,grid,bits,dual)),chosen=rows.find(r=>r.bits===sel);panel.style.display='block';panel.innerHTML=`
